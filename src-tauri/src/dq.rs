...
impl BrowserHistory {
    ...
    pub fn visit_page(&mut self, url: String, title: String) {
        ...
        if let Some(werethereany)=self.browser_timeline.get(self.current_page+1).clone(){
            self.browser_timeline.splice(self.current_page+1.., Vec::new());
        }
        self.browser_timeline.push(Page { url: url, title:title });
        self.current_page=if(self.browser_timeline.len()<1){0}else{self.browser_timeline.len()-1};
    }
...
fn tryout() {
    ...
    println!("{:?}",history);
    history.visit_page("http://example.com/about".to_string(), "About Us".to_string());
    println!("{:?}",history);
