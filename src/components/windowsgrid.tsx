...
interface argprops{
    ...
    setfos?: React.Dispatch<React.SetStateAction<string[]>>;
    setFileOpType?: (type: string) => void;
}
...
export default function WindowsEachFromGrid({message,goto,newtab,populatesearchlist,showthumbnail,setfos,setFileOpType,addmark}:argprops){
    ...
    <ContextMenuTrigger className="h-full w-full overflow-hidden">
      <HoverCard >
        <HoverCardTrigger className="h-full w-full">
            <span className="flex flex-col items-center justify-center w-full h-full p-4 overflow-hidden" onDoubleClick={
            ()=>
            {
              goto(message)
              }
                ...
                <div className="flex items-center justify-center h-full w-full">
                  {IMAGE_TYPES.some(type => message.name.includes(type)) ? (
                    <LazyLoadImage
                      ...
                      src={`${convertFileSrc(message.path)}`}
                    />
                  ) : VIDEO_TYPES.some(type => message.name.includes(type)) ? (
                    <div className="w-full h-full overflow-hidden rounded">
                      <VideoComponent path={message.path} hoverplay={true} className="w-full h-full object-cover" />
                    </div>
                  ) : message.is_dir ? (
                    <Folder className="w-full h-full text-blue-500" />
                  ) : (
                    <FileIcon className="w-full h-full text-gray-500" />
                  )}
                </div>
                <div className="text-center w-full px-2">
                  <span className="font-medium text-sm overflow-hidden text-ellipsis block max-w-full">
                    {message.name}{message.foldercon > 0 ? ` (${message.foldercon})` : ""}
                  </span>
                </div>
              </div>
            </span>
    ...
    <ContextMenuContent className=''>
      ...
      <ContextMenuItem onSelect={(e)=>{
        if (setfos) setfos((old)=>[...old,message.path])
        if (setFileOpType) setFileOpType("copy")
      }}>Copy</ContextMenuItem>
      <ContextMenuItem onSelect={(e)=>{
        if (setfos) setfos((old)=>[...old,message.path])
        if (setFileOpType) setFileOpType("cut")
      }}>Cut</ContextMenuItem>
    </ContextMenuContent>
  </ContextMenu>
...
<GPTchatinterface message={message} setasollama={false}/>
{/* </ResizablePanel>
</ResizablePanelGroup> */}
